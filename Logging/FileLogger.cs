    ...
    public FileLoggingOutput(string fileName)
        {
            ...
            _logOutput = new(
                File.Open(
                    path: fileName, mode: FileMode.Append, access: FileAccess.Write,
    ...
    private void Dispose(bool disposing)
        {
            ...
            catch (Exception? exc)
            {
                var msg = string.Format(
                    format: "Ignoring error closing log file {0} - {1}", arg0: _logFileName,
        ...
        Func<TState, Exception, string> formatter, string category)
        {
            if (exception != null)
            {
                var logMessage = FormatMessage(
                    timestamp: DateTime.UtcNow, logLevel: logLevel, caller: category,
